...
class PTBModel(object):
    def __init__(self, is_training, config, input_):
        ...
        num_steps = input_.num_steps  # LSTM展开的步数
        size = config.hidden_size  # 隐含节点数
        ...
        # 如果在训练状态且要dropout，就在lstm_cell后接一个Dropout层
        if is_training and config.keep_prob < 1:
            def attn_cell():
                return tf.contrib.rnn.DropoutWrapper(
                    lstm_cell(), output_keep_prob=config.keep_prob)

        # 使用RNN堆叠函数将lstm_cell多层堆叠得到cell
    ...
    with sv.managed_session() as session:
        for i in range(config.max_max_epoch):
            ...
            train_perplexity=run_epoch(session,m,eval_op=m._train_op,
                                       verbose=True)
            print("Epoch:%d Train PerPlextiy:%.3f" %(i+1,train_perplexity))
